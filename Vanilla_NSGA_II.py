...
class NSGA_II:
    """  
    Description: Holds all functionality of the NSGA-II Multi-Objective Evolutionary Optimization Algorithm
    ...
    """
    def __init__(self, num_gen, pop_size, cross_prob, MP=0, verbose=True):
        self.NGEN = num_gen
    ...
    def check_results_directory(self):
        ...
        """
        if len(os.listdir("Results/NSGA-II")) > 0:
            ...
            if selection == 'y' or selection == 'yes' or selection == 'Y' or selection == 'YES':
                ...
                os.mkdir(path=f'Results/NSGA-II_{folder_extension}') 
                return f'NSGA-II_{folder_extension}'
        else:
            return 'NSGA-II' 
    ...
    def retrieve_pareto(self, population):
        ...
        indivs = [self.eval.transform_individual(indiv) for indiv in pareto_front]
    ...
    def _RUN(self, seed=None) -> None:
        ...
        print(f'--- Start NSGA-II Run for {self.NGEN} generations and a population of size {self.POP_SIZE} distributing work over {self.MP} cores ---') 
        
        # Set random seed and start time measure for complete algorithm
        random.seed(seed)
        ...
        toolbox.register("mate", tools.cxSimulatedBinaryBounded, low=self.BOUND_L, up=self.BOUND_U, eta=20.0)
        ...
        for gen in range(1, self.NGEN+1):
            ...
            offspring = tools.selTournament(pop, self.POP_SIZE, tournsize=4)
...
if __name__ == '__main__':
    # Initialize and run Non-dominated Sorting Genetic Algorithm II
    nsga = NSGA_II(num_gen=200, 
                   pop_size=20, 
                   cross_prob=0.9, 
                   MP=0, 
                   verbose=True)
